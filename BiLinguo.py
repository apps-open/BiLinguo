...
class Vocabulary(QMainWindow):
    def __init__(self):
        ...
        self.__ui.load_button.clicked.connect(
        ...
        self.__ui.translate_button.clicked.connect(
            self.__show_translate_window)
    ...
    def __table_close(self):
        if self.__table:
            ...
            self.__ui.vocabulary_table.setHorizontalHeaderLabels(
            ...
            self.__msg.show(type_=QMessageBox.Information,
                            buttons=QMessageBox.Ok, text=text, title="Information")
    ...
    def __delete_vocab(self):
        ...
        if name:
            ...
            reply = self.__msg.show(type_=QMessageBox.Warning, buttons=QMessageBox.Yes |
                                    QMessageBox.No, text=text, title="Attention!")
            if reply == QMessageBox.Yes:
                try:
                    self.__db.execute(f"DROP TABLE \"{name}\"")
                    self.__db.execute(f"DROP TABLE \"{name}_statistics\"")
                    self.__db.execute(f"DROP TABLE \"{name}_word_repeats\"")
                    self.__db.execute(
    ...
    def __word_add(self):
        if self.__table:
            self.__ui.vocabulary_table.setRowCount(
                self.__ui.vocabulary_table.rowCount()+1)
        else:
            self.__msg.show(type_=QMessageBox.Information,
                ...
                for row in range(self.__ui.vocabulary_table.rowCount()):
                    ...
                    if word and translation:
                        ...
                        if res and row < self.__rows:
                            ...
                            self.__msg.show(
                                type_=QMessageBox.Information, text=text, title="Information")
    ...
    def __translate_word(self):
        ...
        translate_from = self.__translate_window.ui.translate_from.currentText()[
            :2]
        translate_to = self.__translate_window.ui.translate_to.currentText()[
            :2]
        if word_translate and translate_from and translate_to:
            try:
                output = self.__translate.translate(
    ...
    def __show_translate_window(self):
        ...
        self.__translate_window.ui.translate_button.clicked.connect(
            self.__translate_word)
        self.__translate_window.ui.copy_button.clicked.connect(
            self.__copy_translation)
        self.__translate_window.show()

    def __show_select_vocabulary_window(self):
        self.__select_vocab = Select_Vocab(self.__db)
        self.__select_vocab.ui.open_button.clicked.connect(
    ...
    def __show_manage_vocabulary_window(self):
        ...
        self.__manage_vocab.ui.create_button.clicked.connect(
            self.__create_vocab)
        self.__manage_vocab.ui.delete_button.clicked.connect(
            self.__delete_vocab)
